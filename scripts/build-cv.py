...
set_font("Arial", 9)
text(MARGIN_X, y,
     "KhromenokNV@mail.ru  ·  +7 926 897-32-25  ·  t.me/psykhrometer  ·  Москва",
     color=MUTED)
y -= 5 * mm
hr(y)
y -= 5 * mm
...
summary_lines = [
    ...
    "AI-разработки. 5 продуктов на едином инфраструктурном ядре, ~10 R&D-прототипов.",
]
...
experience = [
    ("2024 — н.в.", "Департамент цифровой трансформации, Severin Development",
     ...
     "5 продуктов на едином ядре (production / пилот / прототип), ~10 R&D-прототипов. "
     "Корпоративный контур: SSO/ADFS, RBAC, аудит."),
...
products = [
    ...
    ("Puls",         "Прототип платформы технического заказчика: реализован PM-кабинет (договоры, табели, графики, проблемы) на масштабной БД; остальные роли — каркас. Связка договор → штатка → табель, портфельная аналитика."),
]
...
edu = [
    ("2021 — 2025", "Аспирантура, ЭБСиГХ", "НИУ МГСУ"),
